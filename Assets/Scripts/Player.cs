...
// Description: Player can jump, moving and attack the monster using step
...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // jump
        if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.Space) && !anim.GetBool("IsJumping") && canMove)
        {
    ...
    {
        // stop speed
        if (ControlFreak2.CF2Input.GetButtonUp("Horizontal") && canMove)
        {
            rigid.velocity = new Vector2(rigid.velocity.normalized.x * 0.5f, rigid.velocity.y);
        }

        // Move speed 
        float h = ControlFreak2.CF2Input.GetAxisRaw("Horizontal");
        rigid.AddForce(Vector2.right * h, ForceMode2D.Impulse);

        // flip
        if (ControlFreak2.CF2Input.GetAxisRaw("Horizontal") < 0)
            spriteRender.flipX = true;
        else if (ControlFreak2.CF2Input.GetAxisRaw("Horizontal") > 0)
